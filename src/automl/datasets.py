...
class BaseVisionDataset(VisionDataset):
    ...
    def __init__(
        ...
        assert split in ["train", "test", "valid", "train_valid"], f"Split {split} not supported"
        if split in ["train", "valid", "train_valid"]:
            # valid still has to use train because there is no valid.csv file
            self._split = 'train'
        else:
            self._split = split
        ...
        if split == 'valid':
            self._image_files = self._image_files[:int(0.2 * len(self._image_files))]
            self._labels = self._labels[:int(0.2 * len(self._labels))]
        elif split == 'train':
            self._image_files = self._image_files[int(0.2 * len(self._image_files)):]
            self._labels = self._labels[int(0.2 * len(self._labels)):]
